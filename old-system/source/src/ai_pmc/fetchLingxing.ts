...
async function withRetry<T>(
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      logger.warn(`[fetchLingxing] ${label} 第${i + 1}次失败，${wait}ms 后重试: ${String(err)}`);
      await sleep(wait);
    }
  }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function mapOrder(raw: any): PurchaseOrder {
...
export async function fetchPurchaseOrders(
  ...
  let offset = 0;
  let pageNum = 0;
  let consecutiveFailures = 0;        // ISSUE-1：连续失败页数
  const MAX_CONSECUTIVE_FAILURES = 5; // 连续失败上限，防止全量不可用时死循环
  ...
  while (true) {
    ...
    try {
      ...
      consecutiveFailures = 0; // 本页成功，重置失败计数
    } catch {
      // ISSUE-1：本页彻底失败 → 跳过本页继续后续，不中断整体（避免漏掉后面的采购单）
      consecutiveFailures++;
      if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
        logger.error(`[fetchLingxing] 连续${consecutiveFailures}页失败，终止本次拉取（已取${results.length}条）`);
        break;
      }
      logger.warn(`[fetchLingxing] 第${pageNum}页(offset=${offset})失败，跳过本页继续（连续失败${consecutiveFailures}）`);
      offset += PAGE_SIZE;
    ...
    if (pageNum === 1) {
      ...
      logger.info(`[fetchLingxing] rawFieldsSnapshot: ${JSON.stringify(snapshot)}`);
    }

    for (const raw of pageData) {
      if (results.length >= maxOrders) break;
      results.push(mapOrder(raw));
    }
  ...
  logger.info(`[fetchLingxing] 前3条: ${JSON.stringify(results.slice(0, 3))}`);
