...
| productList Python mock-only implementation | `approved` / PR、merge TBD；Owner 于 2026-09-12 接受已跑通参考协议作为 mock-only 补充证据，批准用 Python 增加精确 `/erp/sc/routing/data/local_inventory/productList` contract、query-sign adapter、现有 Token Manager reuse 与 MockTransport/synthetic tests；官方 evidence 缺口继续作为真实 validation 风险；若当前 Python dependencies 无 AES 支持，后续 backend PR 可增加 `cryptography`；禁止 Node/npm/JavaScript runtime、真实 Token、真实 ProductLists、P0 sampling、RAW/DB/Redis、frontend/API router 和新 endpoint |
