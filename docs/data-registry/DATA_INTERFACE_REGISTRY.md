...
| `ext-lingxing-product-list-candidate` | Lingxing | 查询本地产品列表 | Outbound business read candidate | POST | `/erp/sc/routing/data/local_inventory/productList` | Query Params signing；official evidence remains partial；Owner-approved reference protocol for Python mock-only implementation | `secret_ref` only | 实现精确 ProductLists mock-only client contract；真实受控验证后置 | 官方 endpoint 未列 store/seller/company/warehouse 参数；账号级实际范围未确认 | Mock-only client contract implementation approved；no real call | None | No in this gate | None | None approved | None | None | None | None | No frontend/API router | Project Owner | `approved` | `PRPs/lingxing-product-list-controlled-validation.md` | `#53` endpoint evidence；`#55` official query-sign evidence；implementation approval PR TBD | 2026-09-12 | Owner 仅批准 Python mock-only query signer、endpoint contract、Token Manager reuse 与 synthetic/MockTransport tests；若当前 Python dependencies 无 AES 支持，后续 backend PR 可增加 `cryptography`；禁止 Node/npm/JavaScript runtime、真实 Token、真实 ProductLists、P0 sampling、RAW/DB/Redis 写入，implementation PR/merge TBD |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
