...
| `ext-lingxing-auth-access-token-candidate` | Lingxing | 获取 access_token 与 refresh_token | Outbound auth request | POST | `/api/auth-server/oauth/access-token` | AppID/AppSecret credential exchange | `secret_ref` only | 为未来后端 Token Manager 获取短期访问令牌 | Enterprise credential scope；不涉及 store scope | On demand, not implemented | TBD | No | None；Token 明文禁止写 RAW | None | None | None | None | None | Backend internal only；permission model TBD | Project Owner | `candidate` | `PRPs/lingxing-token-manager.md` | TBD | 2026-09-10 | 官方契约为 `multipart/form-data`；PRP 仍为 Draft；未获取真实 Token；`expires_in=7199` 仅为示例，不是固定两小时承诺 |
| `ext-lingxing-auth-refresh-token-candidate` | Lingxing | 续约接口令牌 | Outbound auth request | POST | `/api/auth-server/oauth/refresh` | AppID + single-use refreshToken | `secret_ref` only | 为未来后端 Token Manager 刷新访问令牌 | Enterprise credential scope；不涉及 store scope | On demand, not implemented | TBD | No | None；Token 明文禁止写 RAW | None | None | None | None | None | Backend internal only；permission model TBD | Project Owner | `candidate` | `PRPs/lingxing-token-manager.md` | TBD | 2026-09-10 | 官方契约为 `multipart/form-data`；每个 refresh_token 只能使用一次；独立有效期与存储方式官方未确认；真实调用未获批准 |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
