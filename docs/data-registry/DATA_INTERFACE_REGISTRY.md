...
| `ext-lingxing-product-list-candidate` | Lingxing | 查询本地产品列表 | Outbound business read candidate | POST | `/erp/sc/routing/data/local_inventory/productList` | Official Query Params and high-level signing steps captured；deterministic signing contract incomplete | `secret_ref` only | 评估本地产品列表是否适合作为首个 controlled business API validation 目标 | 官方 endpoint 未列 store/seller/company/warehouse 参数；账号级实际范围未确认 | None approved | None | No in this gate | None | None approved | None | None | None | None | No frontend/API router | Project Owner | `blocked` | `PRPs/lingxing-product-list-controlled-validation.md` | TBD | 2026-09-11 | 官方 evidence 已确认 `app_key` 来源为 APP ID、签名字段集合、高层排序/拼接、MD5 uppercase、AES/ECB/PKCS5PADDING、URL encoding 与 2 分钟窗口；复杂 body canonicalization、精确 comparator/字符编码、AES 输出编码、timestamp 单位/时区和安全 expected-sign vector 仍缺失；禁止实现、真实调用、RAW/DB/Redis 写入和 P0 sampling，PR/merge TBD |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
