...
| `ext-lingxing-product-list-candidate` | Lingxing | 查询本地产品列表 | Outbound business read candidate | POST | `/erp/sc/routing/data/local_inventory/productList` | Lingxing Token Manager candidate; no client contract | `secret_ref` only | 评估本地产品列表是否适合作为首个 controlled business API validation 目标 | 当前派生索引未提供 store/seller scope 字段；权限范围未确认 | None approved | None | No in this gate | None | None approved | None | None | None | None | No frontend/API router | Project Owner | `blocked` | `PRPs/lingxing-product-list-controlled-validation.md` | TBD | 2026-09-11 | 仅有非权威 normalized candidate 索引；Content-Type、参数位置、required-field/scope 和可执行限流契约未确认；禁止实现、真实调用、RAW/DB/Redis 写入和 broad P0 sampling，PR/merge TBD |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
