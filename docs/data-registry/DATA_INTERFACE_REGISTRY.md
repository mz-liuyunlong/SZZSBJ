...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `ext-lingxing-p0-raw-candidates` | Lingxing | P0 RAW 采样候选集 | Inbound | POST candidates | TBD；Owner 必须从 migration priority P0 选择精确 endpoint allowlist | Provider credential reference | TBD `secret_ref` only | 为获批 P0 业务响应保存脱敏、可回放 RAW 证据 | Walmart US；精确 store scope 待 Owner 决定 | Bounded capture candidate | None approved | Yes | `storage-raw-lingxing-api-candidate` | None approved | None | None | None | None | TBD；RAW default deny | Project Owner | `candidate` | `PRPs/lingxing-raw-foundation.md` | TBD | 2026-09-10 | 本条不批准真实调用、全量同步或任意 endpoint；认证响应不得保存明文 credential |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
...
| `storage-raw-lingxing-api-candidate` | L2 RAW | `raw_lingxing_api`（候选名） | 保存获批领星业务响应的脱敏、append-only 来源证据 | Evidence only；不是业务权威 | `ext-lingxing-p0-raw-candidates` | Future approved Lingxing RAW writer | Restricted integration/audit reader TBD | Pending Owner decision | High sensitivity；default deny；no credential plaintext | `candidate` | `PRPs/lingxing-raw-foundation.md` | TBD | 表、migration、repository 和 writer 均未实现；不允许前端/业务 API 直查，不允许写 DIM/FACT/Core |
