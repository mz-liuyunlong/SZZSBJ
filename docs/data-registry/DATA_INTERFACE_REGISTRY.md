...
| `ext-lingxing-product-list-candidate` | Lingxing | 查询本地产品列表 | Outbound business read candidate | POST | `/erp/sc/routing/data/local_inventory/productList` | Official query auth fields identified; complete signing algorithm and app_key source mapping pending | `secret_ref` only | 评估本地产品列表是否适合作为首个 controlled business API validation 目标 | 官方 endpoint 未列 store/seller/company/warehouse 参数；账号级实际范围未确认 | None approved | None | No in this gate | None | None approved | None | None | None | None | No frontend/API router | Project Owner | `blocked` | `PRPs/lingxing-product-list-controlled-validation.md` | TBD | 2026-09-11 | Owner Review 确认 ProductLists JSON body/envelope evidence 足以形成未来 mock-only endpoint proposal，但完整 sign algorithm、复杂 body canonicalization、app_key 来源映射和安全 synthetic vector 未入库；禁止实现、真实调用、RAW/DB/Redis 写入和 P0 sampling，PR/merge TBD |
|  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |  |
