...
rule table and `dim_lingxing_stores`. They never request Lingxing, never read ODS / RAW and
never write. The manual-override write route (PRP §7.5) is a separate task (G3-F) and is not
part of this contract. Merging this contract does not authorize any production migration,
DWD publish or DWS refresh; those remain Owner-authorized runbook steps.
...
| Route | Permission |
|---|---|
| all five GET routes below | `pmc:purchase:read` |
...
prefix); no write route is reachable through preview. Money is serialized as a decimal
...
summary, detail, SKU cycles, pending plans on DWS rows produced by the refresh).
