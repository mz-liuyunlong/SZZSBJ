        ...
        return (
            self.session.execute(
                text(
                    ...
                    "and ((cast(:customer_order_id as text) is not null "
                    "and source_order_id=cast(:customer_order_id as text)) "
                    "or (cast(:purchase_order_id as text) is not null "
                    "and source_order_id=cast(:purchase_order_id as text))) "
                    ...
                    "(cast(:local_sku as text) is not null "
                    "and trim(local_sku)=trim(cast(:local_sku as text))) or "
                    "(cast(:msku as text) is not null and trim(msku)=trim(cast(:msku as text)))) "
                    "order by case "
                    "when cast(:item_id as text) is not null and item_id=cast(:item_id as text) then 0 "
                    "when cast(:local_sku as text) is not null "
                    "and trim(local_sku)=trim(cast(:local_sku as text)) then 1 else 2 end,"
    ...
    def _refresh_daily_sales_mart(self) -> int:
        ...
        self.session.execute(
            text(
                ...
                "l.local_name,l.title,l.picture_url,l.platform_code,inv.wfs_available_quantity,"
                ...
                "inv.source_account_ref=s.source_account_ref and inv.snapshot_date_la=s.business_date_la "
