        ...
        total = int(
            self.session.execute(
                text(
                    ...
                    "left join ops_wfs_fee_anomaly_cases c on c.source_account_ref=m.source_account_ref "
        ...
        rows = (
            self.session.execute(
                text(
                    ...
                    "m.wfs_fee_actual_total_amount actual_fee_amount,m.wfs_fee_variance_amount variance_amount,"
                    "m.wfs_fee_variance_rate variance_rate,m.wfs_fee_expected_unit_amount expected_unit_amount,"
                    "case when m.cost_quantity>0 then m.wfs_fee_actual_total_amount/m.cost_quantity end actual_unit_amount,"
                    "coalesce(c.status,'未开Case') status,c.case_no,c.reason,coalesce(c.priority,'中') priority,"
                    "c.claim_amount,coalesce(c.recovered_amount,0) recovered_amount,c.case_opened_at,"
    ...
    def write_actual(
        ...
        self.session.execute(
            text(
                ...
                "actual_fee_amount,currency_code,source_type,source_observed_at,created_at,updated_at) "
                "values (:id,:account,:day,:store_id,:item_id,:msku,:source_line_ref,:actual_fee_amount,"
                ":currency_code,:source_type,:source_observed_at,:now,:now) "
                "on conflict (source_account_ref,source_line_ref) do update set "
                "business_date_la=excluded.business_date_la,store_id=excluded.store_id,item_id=excluded.item_id,"
                "msku=excluded.msku,actual_fee_amount=excluded.actual_fee_amount,currency_code=excluded.currency_code,"
                "source_type=excluded.source_type,source_observed_at=excluded.source_observed_at,updated_at=excluded.updated_at"
            ),
            {
                "id": str(uuid4()),
                "account": payload.source_account_ref,
        ...
        result = self.session.execute(
            text(
                "with actual as (select sum(actual_fee_amount) total from fact_walmart_wfs_fee_actual "
                "where source_account_ref=:account and business_date_la=:day and store_id=:store_id "
                ...
                "then (actual.total-m.wfs_fee_expected_total_amount)/m.wfs_fee_expected_total_amount end,"
                ...
                "and m.first_leg_cost_total_usd is not null and m.storage_fee_total_amount is not null "
                "then m.sales_amount-coalesce(m.ad_spend_amount,0)-coalesce(m.commission_fee_amount,0)-actual.total "
                "-m.purchase_cost_total_usd-m.first_leg_cost_total_usd-m.storage_fee_total_amount end,"
                "gross_margin=case when m.sales_amount>0 and m.purchase_cost_total_usd is not null "
                "and m.first_leg_cost_total_usd is not null and m.storage_fee_total_amount is not null "
                "then (m.sales_amount-coalesce(m.ad_spend_amount,0)-coalesce(m.commission_fee_amount,0)-actual.total "
                "-m.purchase_cost_total_usd-m.first_leg_cost_total_usd-m.storage_fee_total_amount)/m.sales_amount end,"
                "roi=case when coalesce(m.purchase_cost_total_usd,0)+coalesce(m.first_leg_cost_total_usd,0)>0 "
                "and m.purchase_cost_total_usd is not null and m.first_leg_cost_total_usd is not null "
                "and m.storage_fee_total_amount is not null "
                "then (m.sales_amount-coalesce(m.ad_spend_amount,0)-coalesce(m.commission_fee_amount,0)-actual.total "
                "-m.purchase_cost_total_usd-m.first_leg_cost_total_usd-m.storage_fee_total_amount)"
    ...
    def update_case(
        ...
        identity = (
            self.session.execute(
                text(
                    ...
                    "from mart_daily_sales_item_day where id=:id and source_account_ref=any(:accounts)"
                ),
        ...
        self.session.execute(
            text(
                ...
                "claim_amount,recovered_amount,case_opened_at,next_follow_at,latest_follow,updated_by,created_at,updated_at) "
                "values (:id,:account,:day,:store_id,:item_id,:msku,:status,:case_no,:reason,:priority,"
                ":claim_amount,:recovered_amount,:case_opened_at,:next_follow_at,:latest_follow,:updated_by,:now,:now) "
                "on conflict (source_account_ref,business_date_la,store_id,item_id,msku) do update set "
                "status=excluded.status,case_no=excluded.case_no,reason=excluded.reason,priority=excluded.priority,"
                "claim_amount=excluded.claim_amount,recovered_amount=excluded.recovered_amount,"
                "case_opened_at=excluded.case_opened_at,next_follow_at=excluded.next_follow_at,"
                "latest_follow=excluded.latest_follow,updated_by=excluded.updated_by,updated_at=excluded.updated_at"
            ),
            {
                "id": str(uuid4()),
